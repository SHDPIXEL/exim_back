    ...
    try {
        ...
        if (typeof options === 'string') {
            try {
                ...
                return res.json({ status: 'error', message: 'Invalid options format.' });
            }
        }

        // Validate options
        if (!Array.isArray(options) || options.length !== 4) {
            return res.json({ status: 'error', message: 'Please provide exactly 4 options as an array.' });
        }

        // Validate correctAnswerIndex
        if (typeof correctAnswerIndex === 'undefined' || correctAnswerIndex === '') {
            return res.json({ status: 'error', message: 'correctAnswerIndex is required.' });
        }
